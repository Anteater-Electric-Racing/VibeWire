    ...
    <ModalShell title="Tips" onClose={() => setOpen(false)} widthClassName="w-[540px]">
      ...
      <Section title="Canvas tricks">
        <ul className="divide-y divide-transparent">
          ...
          <Tip>Click the triangle on a connector, or double-click / Shift-click the connector, to open or close its pin table. Wires fan out to each cavity; collapse it to bundle them again. A wire that carries only one signal shows that signal name beside the route, unless the signal still has its default &quot;new signal&quot; name — drag the name to move it, double-click it to rename the signal, Delete or × to hide it, and use <span className="text-zinc-100">Show signal name</span> on the selected Harness Bundle to bring it back. A ⤾ button appears next to a moved name to put it back at the default midpoint.</Tip>
          <Tip>Click a Harness Bundle that runs dot-to-dot with exactly one named signal — a simple pass-through wire with nothing else to click into — and it jumps straight into renaming that signal. Type the new name, Enter or click away to confirm, Esc to cancel.</Tip>
          ...
          <Tip>A visual dot is a splice point, not a two-sided cavity — route as many wires onto the same dot as you want, from either side. If every wire already at that dot shares one wire color, the new wire&apos;s signal color starts out matching.</Tip>
