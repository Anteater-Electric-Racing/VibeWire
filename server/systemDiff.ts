/**
 * Pure, deterministic entity diffing for System documents and keyed sidecars.
 *
 * System collections are compared by stable entity id; map documents are
...
export interface DiffableSystem {
  hierarchy?: readonly DiffableEntity[];
  connectors?: readonly DiffableEntity[];
  branchPoints?: readonly DiffableEntity[];
...
function systemEntities(
  system: DiffableSystem | null | undefined,
  label: 'previous' | 'next',
): Map<string, DiffableEntity> {
  const entities = new Map<string, DiffableEntity>();
  for (const collectionName of SYSTEM_COLLECTIONS) {
    const collection = system?.[collectionName] ?? [];
    if (!Array.isArray(collection)) {
      throw new Error(`Cannot diff ${label} System: '${collectionName}' must be an array.`);
    }
    for (const entity of collection) {
      if (!entity || typeof entity !== 'object' || typeof entity.id !== 'string' || !entity.id) {
        throw new Error(
          `Cannot diff ${label} System: '${collectionName}' contains an entity without a valid id.`,
        );
      }
      if (entities.has(entity.id)) {
        throw new Error(`Cannot diff ${label} System: duplicate entity id '${entity.id}'.`);
      }
...
export function diffSystem(
  prev?: DiffableSystem | null,
  next?: DiffableSystem | null,
): EntityDiff {
  return diffMaps(
    systemEntities(prev, 'previous'),
    systemEntities(next, 'next'),
  );
}
